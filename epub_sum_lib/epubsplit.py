...
def _unirepl(match):
    ...
    try:
        ...
        print(
            "Numeric entity translation failed, skipping: &#x%s%s"
...
def removeEntities(text):
    ...
    return (
        text.replace("&", "&amp;").replace("&amp;lt", "&lt;").replace("&amp;gt", "&gt;")
    )
...
class SplitEpub:
    ...
    def get_manifest_items(self):
        if not self.manifest_items:
            ...
            for item in self.get_content_dom().getElementsByTagName("item"):
                fullhref = normpath(
                    unquote(self.get_content_relpath() + item.getAttribute("href"))
                )
                # print("---- item fullhref:%s"%(fullhref))
                self.manifest_items["h:" + fullhref] = (
                    item.getAttribute("id"),
                    item.getAttribute("media-type"),
                )
                self.manifest_items["i:" + item.getAttribute("id")] = (
                    fullhref,
                    item.getAttribute("media-type"),
                )
    ...
    def get_toc_map(self):
        if not self.toc_map:
            ...
            for navpoint in self.get_toc_dom().getElementsByTagName("navPoint"):
                src = normpath(
                    unquote(
                        self.get_content_relpath()
                        + navpoint.getElementsByTagName("content")[0].getAttribute(
                            "src"
                        )
                    )
                )
                ...
                try:
                    text = unicode(
                        navpoint.getElementsByTagName("text")[0].firstChild.data
                    )
                except:
            ...
            if href in self.get_toc_map():
                ...
                for tocitem in self.get_toc_map()[href]:
                    ...
                    if anchor:
                        ...
                        t = splitHtml(
                            self.epub.read(href).decode("utf-8"), anchor, before=False
                        )
                        if len(t) > 1500:
                            t = t[:1500] + "..."
                        current["sample"] = t
        ...
        for fl in outfiles:
            ...
            for a in soup.findAll("a"):
                if a.has_attr("href"):
                    ...
                    if (
                        path in self.filecache.anchors
                        and self.filecache.anchors[path] != path
                    ):
                        a["href"] = self.filecache.anchors[path][
                            len(get_path_part(fl[0])) :
                        ]
    ...
    def write_split_epub(
        ...
        metadata.appendChild(
            newTag(
                contentdom, "dc:identifier", text=uniqueid, attrs={"id": "epubsplit-id"}
            )
        )
        if titleopt is None:
        ...
        for author in useauthors:
            if author not in usedauthors:
                ...
                metadata.appendChild(
                    newTag(
                        contentdom, "dc:creator", attrs={"opf:role": "aut"}, text=author
                    )
                )
        ...
        for filename, id, type, filedata in files:
            ...
            manifest.appendChild(
                newTag(
                    contentdom,
                    "item",
                    attrs={"id": id, "href": filename, "media-type": type},
                )
            )
            spine.appendChild(
                newTag(contentdom, "itemref", attrs={"idref": id, "linear": "yes"})
            )
        ...
        for linked, type in self.filecache.linkedfiles:
            ...
            manifest.appendChild(
                newTag(
                    contentdom,
                    "item",
                    attrs={"id": id, "href": linked, "media-type": type},
                )
            )
        ...
        depthnode = newTag(
            tocncxdom, "meta", attrs={"name": "dtb:depth", "content": "1"}
        )
        head.appendChild(depthnode)
        head.appendChild(
            newTag(
                tocncxdom, "meta", attrs={"name": "dtb:totalPageCount", "content": "0"}
            )
        )
        head.appendChild(
            newTag(
                tocncxdom, "meta", attrs={"name": "dtb:maxPageNumber", "content": "0"}
            )
        )
        ...
        for line in self.split_lines:
            if line["include"]:
                ...
                for title in line["toc"]:
                    ...
                    if (
                        line["anchor"]
                        and line["href"] + "#" + line["anchor"]
                        in self.filecache.anchors
                    ):
                        src = self.filecache.anchors[
                            line["href"] + "#" + line["anchor"]
                        ]
                        # print("toc from anchors(%s#%s)(%s)"%(line['href'],line['anchor'],src))
                    else:
                        # print("toc from href(%s)"%line['href'])
                        src = line["href"]
                    newnav.appendChild(newTag(tocncxdom, "content", {"src": src}))
...
class FileCache:
    ...
    def __init__(self, manifest_items={}):
        ...
        for key, value in six.iteritems(self.manifest_items):
            ...
            if key.startswith("i:") and value[1] in (
...
def main(argv, usage=None):
    ...
    parser = OptionParser(
        usage
        + """ [options] <input epub> [line numbers...]
...
generate an epub with each of the "lines" given included."""
    ...
    parser.add_option(
        ...
        dest="outputopt",
        default="split.epub",
        help="Set OUTPUT file, Default: split.epub",
        metavar="OUTPUT",
    )
    parser.add_option(
        "--output-dir",
    ...
    parser.add_option(
        ...
        dest="titleopt",
        default=None,
        help="Use TITLE as the metadata title.  Default: '<original epub title> Split' or ToC entry with --split-by-section",
        metavar="TITLE",
    )
    parser.add_option(
        "-d",
        ...
        dest="descopt",
        default=None,
        help="Use DESC as the metadata description.  Default: 'Split from <epub title> by <author>'.",
        metavar="DESC",
    )
    parser.add_option(
        "-a",
        ...
        dest="authoropts",
        default=[],
        help="Use AUTHOR as a metadata author, multiple authors may be given, Default: <All authors from original epub>",
        metavar="AUTHOR",
    )
    parser.add_option(
        "-g",
